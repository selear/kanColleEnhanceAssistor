...
function getDbSchema() {
  ...
  let tblAssist = {
    name: TBL_NAME_LIST.ASSIST,
    columns: {
      uid: { primaryKey: true, autoIncrement: true },
      assistName: { notNull: true, dataType: 'string' },
      canUpgrade: { notNull: true, dataType: 'boolean' },
      accessDay: { notNull: true, dataType: 'array' },
      accessDayStr: { notNull: true, dataType: 'string' },
      eid: { notNull: true, dataType: 'number' }
    }
  };

  let tblEquipName = {
    name: TBL_NAME_LIST.EQUIP_NAME,
    columns: {
      eid: { primaryKey: true, dataType: 'number' },
      equipName: { notNull: true, dataType: 'string' }
    }
  };
...
async function initTblAssist() {
  ...
  let tblName = TBL_NAME_LIST.ASSIST;
  ...
  if (dataInsertCount > 0) {
    console.log(`表'${ tblName }'初始化已完成.`);
  } else {
    console.warn(`初始化失败, 表'${ tblName }'没有录入数据.`);
  }
}

async function initTblEquipName() {

  let tblName = TBL_NAME_LIST.EQUIP_NAME;
